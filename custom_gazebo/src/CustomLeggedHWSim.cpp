#include "custom_gazebo/CustomLeggedHWSim.h"
#include <gazebo_ros_control/gazebo_ros_control_plugin.h>

namespace legged {
bool CustomLeggedHWSim::initSim(const std::string& robot_namespace, ros::NodeHandle model_nh, gazebo::physics::ModelPtr parent_model,
...
void CustomLeggedHWSim::readSim(ros::Time time, ros::Duration period) {
  ...
  for (const auto& contact : contactManager_->GetContacts()) {
    ...
    std::string modelName1 = contact->collision1->GetModel()->GetName();
    if(modelName1 == parent_model_->GetName()){
    ...
    std::string modelName2 = contact->collision2->GetModel()->GetName();
    if(modelName2 == parent_model_->GetName()){  
      std::string linkName = contact->collision2->GetLink()->GetName();
      if (name2contact_.find(linkName) != name2contact_.end()) {
        name2contact_[linkName] = true;
      }
    }
...
void CustomLeggedHWSim::writeSim(ros::Time time, ros::Duration period) {
...
void CustomLeggedHWSim::parseImu(XmlRpc::XmlRpcValue& imuDatas, const gazebo::physics::ModelPtr& parentModel) {
...
void CustomLeggedHWSim::parseContacts(XmlRpc::XmlRpcValue& contactNames) {
...
PLUGINLIB_EXPORT_CLASS(legged::CustomLeggedHWSim, gazebo_ros_control::RobotHWSim)
